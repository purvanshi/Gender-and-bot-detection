...
import pickle
SEMCAT_word_concept_dict = pickle.load(open('/home/kovacs19/PAN-bot-detection/pan19-author-profiling-training-2019-01-28/kb/SEMCAT2018_word_concept_dict.p', 'rb'), encoding='utf-8')
semcor_word_concept_dict = pickle.load(open('/home/kovacs19/PAN-bot-detection/pan19-author-profiling-training-2019-01-28/kb/semcor_noun_verb.supersenses.en_word_concept_dict.p', 'rb'), encoding='utf-8')
...
def ProcessFolder(input_directory, output_directory):    
    ...
    if (not os.path.isdir(tweet_directory)):
        os.mkdir(tweet_directory)
    text_file = open(profile_directory+'/'+"features.txt","wt")    
    text_file.close()
...
#ProcessFolder(opt.input_dir+"/en", opt.output_dir+"/en")
...
with open('/home/kovacs19/PAN-bot-detection/Models/ada1.pkl', 'rb') as fid:
...
with open('/home/kovacs19/PAN-bot-detection/Models/ada2.pkl', 'rb') as fid:
...
cols_to_drop = pd.Index(['avg_AUX', 'avg_CONJ', 'avg_EOL', 'avg_NO_TAG', 'avg_SCONJ'])
...
for i in range(len(author_id)):
    ...
    #if botprediction[i] == 'bot':
    #    ET.SubElement(root, "gender").text = botprediction[i]
    #else:        
    #    ET.SubElement(root, "gender").text = genderprediction[i]
   
    #concatenated_string = 'author_id=' + '"' + str(author_id[i]) + '"\n\t' + lang='
    # concatenated_string = 'author_id=' + '"' + str(author_id[i]) + '"\n\t' + 'lang=' + '"en"' + '\n\t' + 'type=' + '"' + botprediction[i] + '"\n\t' + 'gender="'
    # if botprediction[i] == 'bot':
    #    concatenated_string = concatenated_string + botprediction[i]
    # else:        
    #     concatenated_string = concatenated_string + genderprediction[i] 
    # concatenated_string = concatenated_string + '"\n'    

    root = ET.Element('author')
    root.set('id',author_id[i])
    root.set('lang','en')
    root.set('type',botprediction[i])
    if botprediction[i]=='bot':
        temp='bot'
    else:
        temp=genderprediction[i]
    root.set('gender',temp)

    tree = ET.ElementTree(root)
    filename=csv_dir+'/'+str(author_id[i])+'.xml'
    tree.write(filename)

shutil.rmtree(csv_dir+"/profile/")
shutil.rmtree(csv_dir+"/tweet/")
